...
class StandardAddition {
    ...
    run() {
        ...
        const carryBuffer = new PlaceValueTable(0)
        for(let i = 0; i < size; i++) {
            const value1 = this._parcela1.getPlace(i) || 0
            const value2 = this._parcela2.getPlace(i) || 0
            const carry = carryBuffer.getPlace(i) || 0

            const sum = value1 + value2 + carry
            this._total.setPlace(i, sum%10)
            if(sum >= 10) {
                carryBuffer.setPlace(i+1, 1)
            }

            console.log(`${i}: ${carry ? `[${carry}] +` : ''} ${value1} + ${value2} = ${this._total.getPlace(i)} ${sum >= 10 ? '(carry one)' : '' }`)
        }

        if(carryBuffer.getPlace(size)) {
           this._total.setPlace(size, carryBuffer.getPlace(size))
            console.log(`${size}: [${carryBuffer.getPlace(size)}] = ${carryBuffer.getPlace(size)}`)
